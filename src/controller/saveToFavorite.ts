...
import TvModle from '../schema/tvShow';
import axios from 'axios';

const apiUrl = process.env.TV_SHOW;
...
//   let myMovies = []
//   while (1){
//     const movies = await axios.get(`${apiUrl}airing_today?api_key=${process.env.TMDB_KEY}&page=${index}`);
//     console.log(index)
//     if (movies && index <= 200){
...
//           //movie.popular = true;
//           //movie.upComing = true;
//           movie.airing_today = true;
//           //movie.nowPlaying = true;
//           //console.log(filter)
//           myMovies.push(movie)
//           try{
//           await TvModle.findOneAndUpdate({id: filter}, movie, {new: true, upsert: true})
//           } catch(err) {
//             console.log(err)
...
//       }
//       // try{
//       //   await TvModle.insertMany(myMovies);
